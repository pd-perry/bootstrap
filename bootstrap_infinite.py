...
class DirichletFiniteAgent:
    ...
    def sample(self, num_agents, replay_buffer_size):
        indices = torch.randint(low=0, high=replay_buffer_size, size=(num_agents, replay_buffer_size))
        sorted, indices = torch.sort(indices)
        return sorted

    def gather_num_visits(self, sampled_index, env):
        #sampled_index has shape [env, buffer_length]
        num_visits = torch.zeros((self.S, self.A, self.S))
        model_reward = torch.zeros((self.S, self.A))
        buffer = torch.tensor(self.buffer)
        data_list = buffer[env].unsqueeze(0).repeat(self.num_agents, 1, 1).gather(1, sampled_index.unsqueeze(-1).repeat(1, 1, 4).type(torch.int64)) #gathers the data from buffer based on sampled index
        data_list = data_list.reshape(-1, 4)
        for i in data_list:
            num_visits[int(i[0]), int(i[1]), int(i[2])] += 1
            model_reward[int(i[0]), int(i[1])] += i[3]
        ...
        while time_step < num_time_step:
            ...
            for env in range(self.num_envs):
                ...
                if torch.any(
                    ...
                    # inserted bootstrap code
                    bootstrap_index = self.sample(self.num_agents, len(self.buffer[0]))
                    indices = torch.randint(low=0, high=len(self.buffer[0]),
                                                size=(self.num_agents, len(self.buffer[0])))
                    sampled_index = torch.gather(bootstrap_index, dim=-1, index=indices)
                    sampled_num_visits, sampled_model_reward = self.gather_num_visits(sampled_index, env)

                    # update the posterior of the Dirichlet alpha of transitions
                    self.alpha[env] = torch.ones(self.alpha[env].shape) + sampled_num_visits
                    # update the posterior of the Gaussian of rewards, [n_envs, n_S, n_A]
                    count = torch.ones(self.S, self.A) + torch.sum(sampled_num_visits, dim=-1)
                    self.reward_mean[env] = sampled_model_reward / count
                    self.reward_scale[env] = 1 / torch.sqrt(count)
